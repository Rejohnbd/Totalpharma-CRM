    ...
    if (page <= total_pages) {
        ...
        requestGet(buildUrl(admin_url + url, LoadMoreParameters)).done(function (response) {
            ...
            $('[data-col-status-id="' + status_id + '"]').attr(
                'data-total', parseInt($('[data-col-status-id="' + status_id + '"]').attr('data-total')) + $("<ul></ul>").append(response).find('li').length
            );
...
function init_kanban(url, callbackUpdate, connect_with, column_px, container_px, callback_after_load) {
    ...
    if($kanBanCols.length > 0) {
       ...
       $kanBanCols.each(function(index,col) {
        ...
        url += '&' + refreshParams.map(function(el, idx) {
...
function update_kan_ban_total_when_moving(ui,newStatus) {
    if(ui.sender) {
        ...
        if(oldStatusCol.data('status-id') != newStatus){
            ...
            newStatusCol.attr('data-total', parseInt(newStatusCol.attr('data-total')) + 1)
            var oldStatusCol = $(ui.sender.closest('.kan-ban-col'))
            oldStatusCol.attr('data-total', parseInt(oldStatusCol.attr('data-total')) - 1)
        }
    }
}
...
function proposals_pipeline_update(ui, object) {
    if (object === ui.item.parent()[0]) {
        ...
        setTimeout(function () {
            $.post(admin_url + 'proposals/update_pipeline', data).done(function (response) {
                update_kan_ban_total_when_moving(ui,data.status);
                proposals_pipeline();
            });
        }, 200);
...
function add_batch_payment() {
    $('#modal-wrapper').load(admin_url + 'payments/batch_payment_modal', function () {
        ...
        $('.batch_payment_item').each(function () {
            ...
            invoiceLine.filter('select[name$="[paymentmode]"],input[name$="[amount]"]').each(function () {
                var field = $(this);
                field.rules("add", {
                    required: function () {
                        var isRequired = false;
                        var rowFields = field.closest('.batch_payment_item').find('input, select');
                        rowFields.filter('select[name$="[paymentmode]"],input[name$="[transactionid]"],input[name$="[amount]"]').each(function () {
                            if ($(this).val() != '') {
                                isRequired = true;
                            }
